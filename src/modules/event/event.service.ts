    ...
    constructor(
      ...
      async participateInEvent(eventId: number, userId: number): Promise<string> {
        ...
        if (existingUserIndex !== -1) {
          ...
          await this.eventRepository.save(event);
          return `You have successfully removed your participation from the event`;
        } else {
          // User is not participating, add to the event
          event.users.push(user);
          await this.eventRepository.save(event);
          return `You are now participating in the event`;
        }
      }
      ...
      async getEventParticipants(eventId: number): Promise<any[] | undefined> {
        const event = await this.eventRepository
          ...
          .leftJoinAndSelect('event.users', 'user_event') // Corrigé pour joindre correctement les utilisateurs
          .where('event.id = :eventId', { eventId }) // Corrigé la clause where
          .getOne();
      
        return event ? event.users : undefined; // Retourner la liste des utilisateurs ou undefined
      }
